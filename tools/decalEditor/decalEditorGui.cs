...
function DecalEditorGui::onDeleteKey( %this )
{
...
function DecalEditorTreeView::addNodeTree(%this, %nodeName, %parentName)
{
   ...
   if ( %this.findItemByName(%parentName) == 0 )
   {
      ...
      %this.insertItem( %rootId, %parentName, 0, "");
   }
...
function DecalEditorGui::updateDecalPreview( %this, %material )
{
   if( isObject( %material ) )
   {
      if( %material.diffuseMap[0] !$= "" )
         ...
         DecalPreviewWindow-->decalPreview.setBitmap( MaterialEditorGui.searchForTexture( %material.getId(), %material.baseTex[0]) );     
   }
   else
   {
...
function DecalEditorGui::updateInstancePreview( %this, %material )
{
   if( isObject( %material ) )
   {
      if( %material.diffuseMap[0] !$= "" )
         ...
         DecalPreviewWindow-->instancePreview.setBitmap( MaterialEditorGui.searchForTexture( %material.getId(), %material.baseTex[0]) );       
   }
   else
   {
